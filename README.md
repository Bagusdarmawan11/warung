...
- **Beranda**: pemasukan/pengeluaran/profit — bisa difilter Hari Ini/Minggu Ini/Bulan Ini/Tahun Ini/Kustom, total produk, stok kosong, segera kadaluwarsa (dengan filter periode 1/3/6/12 bulan), grafik tren (line/bar, berwarna), **Analisis AI** (Gemini) dengan ringkasan+saran+rekomendasi terstruktur, **produk terlaris** (bisa diurutkan berdasarkan Omzet/Untung/Frekuensi), **Top Pelanggan**, prediksi restock, dan daftar stok menipis/habis — tiap bagian ada tombol **"Lihat Semua"** langsung ke Daftar Barang dengan filter yang sesuai.
- **Kasir**: scan barcode pakai kamera HP/laptop (atau scanner USB/Bluetooth biasa), **foto produk tampil di keranjang & bisa diperbesar** (jadi kasir gampang cek fisik barangnya benar), keranjang belanja, harga bisa diedit per transaksi (untuk produk timbangan, harga diinput/ditampilkan **per kilogram** — bukan per gram, lebih wajar buat mikirnya), **tanggal transaksi bisa diubah** (buat input transaksi kemarin yang lupa dicatat), checkout multi-item yang **atomik**.
- **Barang Masuk**: tambah produk baru (kode + barcode kecil otomatis, langsung bisa diunduh sebagai gambar PNG) atau tambah stok (restock) produk lama, dengan **tanggal masuk/restock yang bisa diubah** (tidak harus selalu hari ini). Bisa tambahkan **foto produk** (opsional).
- **Produk timbangan (gram)**: untuk barang seperti telur, kemiri, lada, dll yang dibeli curah lalu direpack — input **berat dalam gram** (misal 1000 = 1kg) saat barang masuk, dan saat terjual kasir tinggal input berat yang dibeli pembeli, tanpa perlu input jumlah butir/pack. **Harga diinput per kilogram** (bukan per gram) di semua tempat — Barang Masuk, Kasir, edit harga batch — sistem otomatis mengonversinya di baliknya. Sisa stok/riwayat selalu ditampilkan dengan satuan yang jelas (gram/kg), otomatis dikonversi ke kg kalau ≥1000 gram.
...
### Barang Masuk
- **Produk Baru**: isi nama, jenis satuan (pcs/gram), jumlah, harga modal & jual, **tanggal barang masuk** (default hari ini, bisa diubah untuk input yang lupa dicatat), kadaluwarsa (opsional), dan **foto produk** (opsional) → kode & barcode otomatis dibuat, langsung ada tombol **Unduh Barcode** (file gambar PNG kecil, siap ditempel/cetak sendiri di stiker — tanpa nama/harga, cuma barcode polos seperti di kemasan produk pada umumnya).
- **Tambah Stok**: cari produk yang sudah ada, isi jumlah tambahan (boleh harga beda dari sebelumnya), **tanggal restock** bisa diubah juga → otomatis jadi batch baru, barcode/kode produk tetap sama seperti sebelumnya (tidak perlu unduh ulang / stiker baru).

### Produk (Daftar Barang)
- **Tekan nama/baris produk** untuk buka **popup detail** — info stok, harga modal, harga jual, estimasi untung, tanggal masuk terakhir, dan kadaluwarsa ditampilkan rinci di bagian atas, dengan tombol **Edit** terpisah di bawah yang membuka popup edit sendiri (nama/kategori, **jenis satuan**, koreksi stok manual, riwayat harga per batch, hapus produk) — jadi popup info tetap bersih. Ada juga tab **Riwayat Transaksi** (semua pergerakan masuk/keluar produk itu, lengkap dengan pembeli & untung per transaksi, satuannya selalu jelas gram/kg/pcs).
- **Salah pilih jenis satuan saat bikin produk?** Buka Edit Produk → ubah dropdown "Jenis Satuan" → sistem akan konfirmasi dulu (karena ini tidak mengonversi angka stok yang sudah ada, cuma mengubah cara sistem membacanya — pastikan angka stoknya memang sudah sesuai arti satuan barunya sebelum diubah).
...
- Filter chip: Semua / Stok Bermasalah / Stok Menipis / Stok Habis / Segera Kadaluwarsa.
...
- **Produk timbangan (gram) lewat import**: kalau di data lama kamu ada produk yang sebenarnya satuan timbangan (misal telur, kemiri) — pastikan semua barisnya (baik di Barang Masuk maupun Penjualan) pakai **nama produk yang PERSIS sama**, kolom Qty diisi **gram langsung** (misal 1000 untuk 1kg), dan kolom harga diisi **harga per kilogram**. Lalu di halaman Import, isi kotak "Nama produk satuan timbangan/gram" dengan nama produk itu (satu per baris) — sistem otomatis membuatnya sebagai produk gram dan mengonversi harganya.
...
- Semua produk hasil import dibuat sebagai satuan **pcs**, KECUALI nama produknya kamu daftarkan di kotak "Nama produk satuan timbangan/gram" saat import (lihat bagian Import di atas) — cara ini yang disarankan untuk produk timbangan seperti telur/kemiri di data baru kamu (samakan namanya, isi Qty dalam gram, harga per kg). Kalau ada produk yang terlanjur salah dibuat sebagai pcs padahal seharusnya gram (atau sebaliknya), buka Edit Produk → ubah dropdown "Jenis Satuan"-nya langsung (lihat bagian Produk di atas) — tidak perlu bikin ulang dari nol. Sistem **tidak** otomatis menggabungkan produk dengan nama BERBEDA (misal "Telur Ayam 1 Kg" vs "Telur Ayam 2 KG" tetap dianggap 2 produk terpisah) — hanya produk dengan nama PERSIS SAMA (huruf besar/kecil diabaikan) yang otomatis dianggap satu produk & direstock sebagai batch tambahan.
