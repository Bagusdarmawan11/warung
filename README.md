...
- **Produk timbangan (gram)**: untuk barang seperti telur, kemiri, lada, dll yang dibeli curah lalu direpack — input **berat dalam gram** (misal 1000 = 1kg) saat barang masuk, dan saat terjual kasir tinggal input berat yang dibeli pembeli, tanpa perlu input jumlah butir/pack. **Harga diinput per kilogram** (bukan per gram) di semua tempat — Barang Masuk, Kasir, edit harga batch, gabung produk — sistem otomatis mengonversinya di baliknya. Sisa stok ditampilkan dalam gram/kg, bukan jumlah satuan.
- **Gabung Produk**: kalau ada beberapa produk yang seharusnya satu (misal karena data lama beda-beda nama), pilih semuanya (tekan & tahan salah satu, lalu ketuk yang lain) lalu **Gabung** jadi satu produk timbangan baru. Produk lama otomatis diarsipkan (bukan dihapus), riwayat transaksi lama tetap aman.
...
- **Riwayat**: nomor urut, pagination, klik transaksi untuk detail (pembeli, keuntungan, lama barang di stok dalam hari, sisa stok), unduh **PDF** laporan penjualan.
...
- Semua produk hasil import dibuat sebagai satuan **pcs**, KECUALI nama produknya kamu daftarkan di kotak "Nama produk satuan timbangan/gram" saat import (lihat bagian Import di atas) — cara ini yang disarankan untuk produk timbangan seperti telur/kemiri di data baru kamu (samakan namanya, isi Qty dalam gram, harga per kg). Untuk data lama yang sudah terlanjur kepecah jadi beberapa produk beda nama (misal "Telur Ayam 1 Kg" vs "Telur Ayam 2 KG"), masih bisa dirapikan pakai fitur **Gabung Produk** di halaman Produk (tekan & tahan beberapa produk yang mau digabung) untuk menyatukannya jadi satu produk gram. Sistem **tidak** otomatis menggabungkan produk dengan nama BERBEDA — hanya produk dengan nama PERSIS SAMA (huruf besar/kecil diabaikan) yang otomatis dianggap satu produk & direstock sebagai batch tambahan.
